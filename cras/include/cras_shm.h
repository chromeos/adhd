 ...
 */
int cras_shm_info_init(const char* shm_name,
                       uint32_t length,
 ...
 * info_out - pointer where the created cras_shm_info will be stored.
 */
int cras_shm_info_init_with_fd(int fd,
                               size_t length,
                               struct cras_shm_info* info_out);
...
static inline unsigned cras_shm_get_curr_read_frames(
    const struct cras_audio_shm* shm) {
...
static inline uint8_t* cras_shm_get_read_buffer_base(
    const struct cras_audio_shm* shm) {
...
static inline uint8_t* cras_shm_get_write_buffer_base(
    const struct cras_audio_shm* shm) {
...
static inline uint8_t* cras_shm_get_writeable_frames(
    const struct cras_audio_shm* shm,
 ...
 */
static inline uint8_t* cras_shm_get_readable_frames(
    const struct cras_audio_shm* shm,
    size_t offset,
    size_t* frames) {
  unsigned buf_idx = shm->header->read_buf_idx & CRAS_SHM_BUFFERS_MASK;
...
static inline size_t cras_shm_get_bytes_queued(
    const struct cras_audio_shm* shm) {
  size_t total, i;
...
static inline int cras_shm_is_buffer_available(
    const struct cras_audio_shm* shm) {
...
static inline int cras_shm_check_write_overrun(struct cras_audio_shm* shm) {
  int ret = 0;
  size_t write_buf_idx = shm->header->write_buf_idx & CRAS_SHM_BUFFERS_MASK;
  ...
  if (!shm->header->write_in_progress[write_buf_idx]) {
    ...
    if (shm->header->write_offset[write_buf_idx]) {
      ...
      shm->header->overrun_frames += cras_shm_get_curr_read_frames(shm);
      ret = 1;
    }
...
static inline unsigned int cras_shm_frames_written(
    const struct cras_audio_shm* shm) {
  size_t buf_idx = shm->header->write_buf_idx & CRAS_SHM_BUFFERS_MASK;
...
static inline float cras_shm_get_volume_scaler(
    const struct cras_audio_shm* shm) {
...
static inline unsigned cras_shm_overrun_frames(
    const struct cras_audio_shm* shm) {
 ...
 * hardware buffer. */
static inline void cras_shm_update_dropped_samples_duration(
    struct cras_audio_shm* shm,
    struct timespec duration) {
  struct timespec current;
  cras_timespec_to_timespec(&current, &shm->header->dropped_samples_duration);
...
static inline struct cras_timespec cras_shm_dropped_samples_duration(
    const struct cras_audio_shm* shm) {
 ...
 * data in the buffer */
static inline struct cras_timespec cras_shm_underrun_duration(
    const struct cras_audio_shm* shm) {
