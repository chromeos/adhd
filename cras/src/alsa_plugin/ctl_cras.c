...
static int ctl_cras_elem_list(snd_ctl_ext_t* ext_ctl,
                              unsigned int offset,
...
static int ctl_cras_get_attribute(snd_ctl_ext_t* ext_ctl,
...
static int ctl_cras_read_integer(snd_ctl_ext_t* ext_ctl,
                                 snd_ctl_ext_key_t key,
                                 long* value) {
  struct ctl_cras* cras = (struct ctl_cras*)ext_ctl->private_data;
  ...
  switch (key) {
    ...
    case CTL_CRAS_MIXER_PLAYBACK_VOLUME:
      num_nodes =
          get_nodes(cras->client, CRAS_STREAM_OUTPUT, nodes, MAX_IONODES);
...
static int ctl_cras_write_integer(snd_ctl_ext_t* ext_ctl,
                                  snd_ctl_ext_key_t key,
                                  long* value) {
  struct ctl_cras* cras = (struct ctl_cras*)ext_ctl->private_data;
  struct cras_ionode_info nodes[MAX_IONODES];
    ...
    case CTL_CRAS_MIXER_PLAYBACK_VOLUME:
      ...
      for (i = 0; i < num_nodes; i++) {
        ...
        cras_client_set_node_volume(
            cras->client,
            cras_make_node_id(nodes[i].iodev_idx, nodes[i].ionode_idx), *value);
      }
      break;
    default:
      return -EINVAL;
