...
int cras_audio_shm_create(struct cras_shm_info* header_info,
  ...
  shm->header = mmap(NULL, shm->header_info.length, PROT_READ | PROT_WRITE,
  ...
  shm->samples = mmap(NULL, shm->samples_info.length, samples_prot, MAP_SHARED,
                      shm->samples_info.fd, 0);
  if (shm->samples == (uint8_t*)-1) {
    ret = -errno;
    syslog(LOG_ERR, "cras_shm: mmap failed to map shm for samples.");
    goto free_shm;
  }
...
static void cras_shm_restorecon(int fd) {
  ...
  if (snprintf(fd_proc_path, sizeof(fd_proc_path), "/proc/self/fd/%d", fd) <
      0) {
    syslog(LOG_WARNING, "Couldn't construct proc symlink path of fd: %d", fd);
    return;
  }
  ...
  if (path == NULL) {
    syslog(LOG_WARNING, "Couldn't run realpath() for %s: %s", fd_proc_path,
           cras_strerror(errno));
    return;
  }
...
int cras_shm_reopen_ro(const char* name, int fd) {
  ...
  if (fd < 0) {
    fd = -errno;
    syslog(LOG_WARNING, "Failed to re-open shared memory '%s' read-only: %s",
...
void* cras_shm_setup(const char* name,
  ...
  // mmap shm.
  void* exp_state =
      mmap(NULL, mmap_size, PROT_READ | PROT_WRITE, MAP_SHARED, rw_shm_fd, 0);
  if (exp_state == (void*)-1) {
    return NULL;
  }
