...
void show_edid_dtd(FILE* outfile, unsigned char* base) {
  ...
  if (pelclk != 0) {
    ...
    fprintf(outfile, "%dx%d%c@%d.%02d, dot clock %d  %cHsync %cVsync\n", hres,
            vres, (mdflg & 0x80) ? 'i' : 'p', refr, refd, pelclk * 10000,
            (mdflg & 0x2) ? '+' : '-', (mdflg & 0x4) ? '+' : '-');
  ...
  switch (base[DTD_TYPETAG]) {
    ...
    case DTDTYPE_STRING:
    case DTDTYPE_NAME:
      get_dtd_string((const char*)base + DTD_STRING, monstr, DTD_SIZE);
    ...
    case DTDTYPE_LIMITS:
      fprintf(outfile, "V %d - %d Hz, H %d - %d kHz, Pel <= %d MHz\n",
              base[DTD_MINV_HZ], base[DTD_MAXV_HZ], base[DTD_MINH_kHZ],
              base[DTD_MAXH_kHZ], base[DTD_MAXCLK_100kHZ] * 10);
      break;
...
static inline void show_audio_dbc(FILE* outfile,
  ...
  while (dbp < (dbc + db_len + 1)) {
    ...
    fprintf(outfile,
            "Audio: %d channels %s: ", (edid_ext[dbp + DBCA_FORMAT] & 0x7) + 1,
...
static inline void show_vendor_dbc(FILE* outfile,
  ...
  fprintf(
      ...
      "Support: %s%s%s%s%s%s\n",
      edid_ext[dbp + DBCVHDMI_CEC_LO] + (edid_ext[dbp + DBCVHDMI_CEC_HI] << 8),
  ...
  if (edid_ext[dbp + 7] & 0x40) {
    fprintf(outfile, "Interlaced Video latency %dms, audio latency %dms\n",
...
static void show_extended_dbc(FILE* outfile,
  ...
  switch (edid_ext[dbp + DBC_ETAG]) {
    ...
    default:
      fprintf(outfile, "Unknown extended tag data block 0x%x,  length 0x%x\n",
...
void show_cea_timing(FILE* outfile, unsigned char* edid_ext) {
  ...
  fprintf(outfile, "Block has DTDs starting at offset %d (%d bytes of DBCs)\n",
  ...
  fprintf(outfile, "Support: %sbasic audio, %sYCrCb 4:4:4, %sYCrCb 4:2:2\n",
  ...
  while (dbc < off_dtd) {
    ...
    switch (edid_ext[dbc + DBC_TAG_LENGTH] >> DBC_TAG_SHIFT) {
      ...
      default:
        fprintf(outfile, "Unknown Data Block type tag 0x%x, len 0x%x\n",
...
int edid_valid(const unsigned char* edid_data) {
  return (
      (edid_data[EDID_HDR + 0] == 0x00) && (edid_data[EDID_HDR + 1] == 0xff) &&
...
void show_edid(FILE* outfile, unsigned char* edid_data, int ext) {
  ...
  fprintf(outfile, "Manufactured wk %d of %d. Edid version %d.%d\n",
          edid_data[EDID_MFG_WEEK], 1990 + edid_data[EDID_MFG_YEAR], edidver,
          edidrev);
  fprintf(outfile,
  ...
  if ((ext >= 1) && (edid_data[EDID_EXT_FLAG] >= 1)) {
    ...
    if ((eext_tag == 0x02) && (edid_ext[EEXT_REV] == 0x03)) {
      ...
      fprintf(outfile, "EDID %s ext tag 0x%02x rev 0x%02x skipped\n", tagtype,
...
int edid_get_monitor_name(const unsigned char* edid_data,
  ...
  for (i = 0; i < EDID_N_DTDS; i++) {
    ...
    if (dtd[DTD_PCLK_LO] == 0x00 && dtd[DTD_PCLK_HI] == 0x00 &&
      ...
      get_dtd_string((const char*)dtd + DTD_STRING, buf, buf_size);
      return 0;
    }
  }
...
struct edid_device_id edid_get_device_id(const unsigned char* edid_data) {
  struct edid_device_id id = {
      .mfg_id = {edid_data[EDID_MFG_EID], edid_data[EDID_MFG_EID + 1]},
      .prod_code =
          edid_data[EDID_MFG_PROD_LO] + (edid_data[EDID_MFG_PROD_HI] << 8),
      .serial =
          (edid_data[EDID_MFG_SERIAL] + (edid_data[EDID_MFG_SERIAL + 1] << 8u) +
