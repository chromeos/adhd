						     ...
						     const size_t sample_size)
{
	return buf && (sample_size != 0) &&
	       ...
	       (buf->write_idx % sample_size == 0) &&
	       (buf->level % sample_size == 0);
}
...
sample_buffer_weak_ref(struct byte_buffer *ref_buf, const size_t sample_size)
{
	...
	struct sample_buffer buf = { .sample_size = sample_size,
				     .buf = ref_buf };
	return buf;
}
 ...
 */
static inline unsigned int sample_buf_readable(const struct sample_buffer *buf)
{
...
sample_buf_read_pointer_size(struct sample_buffer *buf,
 ...
 */
static inline unsigned int sample_buf_writable(const struct sample_buffer *buf)
{
 ...
 */
static inline unsigned int sample_buf_available(const struct sample_buffer *buf)
{
 ...
 */
static inline uint8_t *
sample_buf_write_pointer_size(struct sample_buffer *buf,
 ...
 */
static inline int
sample_buf_full_with_zero_read_index(struct sample_buffer *buf)
{
