...
{
	struct am_context *am_context = static_cast<struct am_context *>(
		calloc(1, sizeof(struct am_context)));
	if (!am_context) {
	...
	if (const TfLiteStatus status =
		    am_context->interpreter->AllocateTensors();
	    status != kTfLiteOk) {
	       ...
	       const size_t num_outputs)
{
	...
	float *input_buf =
		am_context->interpreter->typed_input_tensor<float>(0);
	if (!input_buf) {
