...
static void emphasis_stage_roots(float gain,
...
static void emphasis_stage_pair_biquads(float gain,
                                        float f1,
                                        float f2,
...
void drc_process(struct drc* drc, float** data, int frames) {
  ...
  // Crossover
  crossover2_process(&drc->xo2, frames, data[0], data[1], data1[0], data1[1],
                     data2[0], data2[1]);
...
int drc_convert_params_to_blob(struct drc* drc,
  ...
  drc_config->enable_emp_deemp = drc->emphasis_disabled ? 0 : 1;
   ...
   */
  int ret;
  ret = eq2_convert_channel_response(drc->emphasis_eq,
                                     (int32_t*)drc_config->emp_coef, 0);
  if (ret < 0) {
    free(drc_config);
    return ret;
  }
  ret = eq2_convert_channel_response(drc->deemphasis_eq,
  ...
  ret = crossover2_convert_params_to_blob(&drc->xo2,
