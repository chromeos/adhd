...
static double tp_diff(struct timespec* tp2, struct timespec* tp1) {
  return (tp2->tv_sec - tp1->tv_sec) + (tp2->tv_nsec - tp1->tv_nsec) * 1e-9;
}

void process(struct crossover2* xo2,
             int count,
             float* data0L,
             float* data0R,
             float* data1L,
  ...
  for (start = 0; start < count; start += 2048) {
    crossover2_process(xo2, min(2048, count - start), data0L + start,
                       data0R + start, data1L + start, data1R + start,
...
int main(int argc, char** argv) {
  ...
  if (argc != 3 && argc != 6) {
    printf(
        "Usage: crossover2_test input.raw output.raw "
  ...
  process(&xo2, frames, data0, data0 + frames, data1, data1 + frames, data2,
          data2 + frames);
