...
void dsp_util_deinterleave_reference(int16_t* input,
                                     float* const* output,
                                     int channels,
                                     int frames) {
...
void dsp_util_interleave_reference(float* const* input,
                                   int16_t* output,
                                   int channels,
                                   int frames) {
...
void TestRounding(float in, int16_t expected, int samples) {
  ...
  dsp_util_interleave_reference(out_floats_ptr_c, out_shorts_c, 2, samples);

  // measure optimized interleave
  for (i = 0; i < ITERATIONS; ++i) {
    dsp_util_interleave(out_floats_ptr_c, (uint8_t*)out_shorts_opt, 2,
  ...
  printf("test interleave compare %6d, %10f %13f %6d %6d %6d %s\n", max_diff,
         in, in * 32768.0f, out_shorts_c[0], out_shorts_opt[0], expected,
         max_diff == 0
             ? "PASS"
             : (out_shorts_opt[0] == expected ? "EXPECTED DIFFERENCE"
                                              : "UNEXPECTED DIFFERENCE"));

  // measure reference C deinterleave
  dsp_util_deinterleave_reference(in_shorts, out_floats_ptr_c, 2, samples);
...
int main(int argc, char** argv) {
  ...
  printf(
      "test interleave compare maxdif,     float,   float * 32k      "
      "C   SIMD expect pass\n");
...
#define EXPECTED_NAN_RESULT 0
  ...
  for (samples = MAXSAMPLES; samples >= MINSAMPLES; samples /= 2) {
    ...
    d = memcmp(out_shorts_c, out_shorts_opt, MAXSAMPLES * 2 * 2 + PAD);
    if (d) {
      printf("interleave compare %d, %d %d, %d %d\n", d, out_shorts_c[0],
             out_shorts_c[1], out_shorts_opt[0], out_shorts_opt[1]);
    }
  }
