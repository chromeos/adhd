...
int cras_helper_add_stream_simple(struct cras_client* client,
  ...
  params = cras_client_unified_params_create(
      CRAS_STREAM_OUTPUT, 2048, CRAS_STREAM_TYPE_DEFAULT, 0, user_data,
  ...
  rc = cras_client_add_pinned_stream(client, dev_idx, stream_id_out, params);
...
int cras_helper_play_buffer(struct cras_client* client,
  ...
  return cras_helper_add_stream_simple(
      client, CRAS_STREAM_OUTPUT, data, play_buffer_callback, play_buffer_error,
      format, frame_rate, num_channels, dev_idx, &stream_id);
}
