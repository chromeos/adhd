...
void plc_experiment(const char* input_filename, bool* pl_seq, bool with_plc) {
  ...
  if (with_plc) {
    snprintf(output_filename, sizeof(output_filename), "output_with_plc.raw");
  } else {
    snprintf(output_filename, sizeof(output_filename), "output_with_zero.raw");
  }
  ...
  while (1) {
    ...
    msbc_input->encode(msbc_input, buffer, MSBC_CODE_SIZE, packet_buffer,
                       MSBC_PKT_FRAME_LEN, &encoded);

    if (pl_seq[count]) {
      if (with_plc) {
        cras_msbc_plc_handle_bad_frames(plc, msbc_output, buffer);
        decoded = MSBC_CODE_SIZE;
      } else {
        msbc_output->decode(msbc_output, msbc_zero_frame, MSBC_PKT_FRAME_LEN,
                            buffer, MSBC_CODE_SIZE, &decoded);
      }
    } else {
      msbc_output->decode(msbc_output, packet_buffer, MSBC_PKT_FRAME_LEN,
                          buffer, MSBC_CODE_SIZE, &decoded);
...
static void show_usage() {
  printf(
      "This test only supports reading/writing raw audio with format:\n"
      "\t16000 sample rate, mono channel, S16_LE\n");
  printf("--help - Print this usage.\n");
  printf("--input_file - path to an audio file.\n");
  printf(
      "--pattern - Hex string representing consecutive packets'"
      "status.\n");
...
int main(int argc, char** argv) {
  ...
  while (true) {
    option_character =
        getopt_long(argc, argv, "i:r:p:h", long_options, &option_index);
