...
static int cras_a2dp_get_capabilities(struct cras_bt_endpoint* endpoint,
  ...
  *len = sizeof(*sbc_caps);
  ...
  sbc_caps->frequency = SBC_SAMPLING_FREQ_16000 | SBC_SAMPLING_FREQ_32000 |
                        SBC_SAMPLING_FREQ_44100 | SBC_SAMPLING_FREQ_48000;
  sbc_caps->allocation_method = SBC_ALLOCATION_SNR | SBC_ALLOCATION_LOUDNESS;
...
void cras_a2dp_start(struct cras_bt_device* device) {
  ...
  if (connected_a2dp.iodev) {
    syslog(LOG_WARNING, "Replacing existing endpoint configuration");
