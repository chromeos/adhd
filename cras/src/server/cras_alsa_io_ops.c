...
inline struct cras_iodev *cras_alsa_iodev_ops_create(
	struct cras_alsa_iodev_ops *ops, size_t card_index,
	const char *card_name, size_t device_index, const char *pcm_name,
...
{
	assert(ops->create);
	return ops->create(card_index, card_name, device_index, pcm_name,
			   dev_name, dev_id, card_type, is_first, mixer, config,
			   ucm, hctl, direction, usb_vid, usb_pid,
			   usb_serial_number);
}

inline int
cras_alsa_iodev_ops_legacy_complete_init(struct cras_alsa_iodev_ops *ops,
...
inline int
cras_alsa_iodev_ops_ucm_add_nodes_and_jacks(struct cras_alsa_iodev_ops *ops,
					    struct cras_iodev *iodev,
...
inline void
cras_alsa_iodev_ops_ucm_complete_init(struct cras_alsa_iodev_ops *ops,
				      struct cras_iodev *iodev)
{
