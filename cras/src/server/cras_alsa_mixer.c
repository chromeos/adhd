				   ...
				   snd_mixer_elem_t *elem)
{
	if (snd_mixer_selem_has_playback_volume(elem)) {
		...
		if (c->max_volume_dB != MIXER_CONTROL_VOLUME_DB_INVALID) {
		       ...
		       snd_mixer_elem_t *elem)
{
	...
	rc = mixer_control_create(&c, NULL, elem, dir);
	if (rc)
		return rc;
				...
				struct mixer_name *coupled_controls)
{
	...
	rc = mixer_control_create_by_name(
		 &control, cmix, name, coupled_controls, dir);
	if (rc)
		return rc;
	...
	if (control->has_volume)
		syslog(LOG_DEBUG, "Control '%s' volume range: [%ld:%ld]",
		       control->name, control->min_volume_dB,
		       control->max_volume_dB);

	if (dir == CRAS_STREAM_OUTPUT) {
		control->volume_curve =
			create_volume_curve_for_output(cmix, control->name);
		DL_APPEND(cmix->output_controls, control);
	}
	else if (dir == CRAS_STREAM_INPUT) {
		DL_APPEND(cmix->input_controls, control);
	}
	return 0;
}
		...
		struct mixer_control *mixer_input)
{
	...
	DL_FOREACH(cmix->main_capture_controls, c)
		if (c->min_volume_dB != MIXER_CONTROL_VOLUME_DB_INVALID)
			total_min += c->min_volume_dB;
	if (mixer_input &&
	    mixer_input->min_volume_dB != MIXER_CONTROL_VOLUME_DB_INVALID)
		...
		struct mixer_control *mixer_input)
{
	...
	if (mixer_input &&
	    mixer_input->max_volume_dB != MIXER_CONTROL_VOLUME_DB_INVALID)
