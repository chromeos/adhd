...
static bool usb_noise_cancellation_support_is_exposed(uint32_t dev_idx,
						      uint32_t node_idx)
{
...
static int usb_open_dev(struct cras_iodev *iodev)
{
	...
	if (iodev->active_node->audio_effect & EFFECT_TYPE_NOISE_CANCELLATION) {
		...
		if (!usb_noise_cancellation_support_is_exposed(
...
static void usb_update_active_node(struct cras_iodev *iodev, unsigned node_idx,
				   unsigned dev_enabled)
{
	...
	usb_alsa_iodev_set_active_node(iodev, first_plugged_node(iodev),
				       dev_enabled);
}
...
static struct alsa_usb_output_node *
usb_get_active_output(const struct alsa_usb_io *aio)
{
	return (struct alsa_usb_output_node *)aio->base.active_node;
}

static struct alsa_usb_input_node *
usb_get_active_input(const struct alsa_usb_io *aio)
{
...
usb_get_curve_for_output_node(const struct alsa_usb_io *aio,
 ...
 */
static const struct cras_volume_curve *
usb_get_curve_for_active_output(const struct alsa_usb_io *aio)
{
...
static int usb_get_ucm_flag_integer(struct alsa_usb_io *aio,
				    const char *flag_name, int *result)
{
	char *value;
	int i;
...
static void
usb_set_output_node_software_volume_needed(struct alsa_usb_output_node *output,
					   struct alsa_usb_io *aio)
{
	...
	cras_alsa_mixer_get_playback_dBFS_range(mixer, output->mixer_output,
						&max, &min);
...
static void
usb_set_input_node_intrinsic_sensitivity(struct alsa_usb_input_node *input,
					 struct alsa_usb_io *aio)
{
	long sensitivity;
	int rc;
 ...
 */
static struct alsa_usb_output_node *
usb_new_output(struct alsa_usb_io *aio, struct mixer_control *cras_control,
	       const char *name)
{
	...
	usb_check_auto_unplug_output_node(aio, &output->base,
					  output->base.plugged);
	return output;
}
...
static struct alsa_usb_input_node *
usb_new_input(struct alsa_usb_io *aio, struct mixer_control *cras_input,
	      const char *name)
{
	...
	usb_check_auto_unplug_input_node(aio, &input->base,
					 input->base.plugged);
	return input;
}
 ...
 */
static struct alsa_usb_output_node *
usb_get_output_node_from_jack(struct alsa_usb_io *aio,
			      const struct cras_alsa_jack *jack)
{
	struct mixer_control *mixer_output;
...
static struct alsa_usb_input_node *
usb_get_input_node_from_jack(struct alsa_usb_io *aio,
			     const struct cras_alsa_jack *jack)
{
	struct mixer_control *mixer_input;
	struct cras_ionode *node = NULL;
...
static const struct cras_alsa_jack *
usb_get_jack_from_node(struct cras_ionode *node)
{
...
usb_create_volume_curve_for_jack(const struct cras_card_config *config,
				 const struct cras_alsa_jack *jack)
{
	struct cras_volume_curve *curve;
	const char *name;
				       ...
				       int plugged, void *arg)
{
	...
	if (!node->jack) {
		...
		if (node->volume_curve == NULL)
			node->volume_curve = usb_create_volume_curve_for_jack(
				aio->config, jack);
	}
 ...
 */
static void usb_set_iodev_name(struct cras_iodev *dev, const char *card_name,
			       const char *dev_name, size_t card_index,
			       size_t device_index,
			       enum CRAS_ALSA_CARD_TYPE card_type,
			       size_t usb_vid, size_t usb_pid,
			       char *usb_serial_number)
{
	...
	dev->info.stable_id = SuperFastHash(
		(const char *)&usb_vid, sizeof(usb_vid), dev->info.stable_id);
	dev->info.stable_id = SuperFastHash(
		(const char *)&usb_pid, sizeof(usb_pid), dev->info.stable_id);
	dev->info.stable_id =
		SuperFastHash(usb_serial_number, strlen(usb_serial_number),
			      dev->info.stable_id);
...
static int usb_get_fixed_rate(struct alsa_usb_io *aio)
{
	...
	if (aio->base.direction == CRAS_STREAM_OUTPUT) {
		struct alsa_usb_output_node *active =
			usb_get_active_output(aio);
		if (!active)
			return -ENOENT;
...
static unsigned int usb_get_num_severe_underruns(const struct cras_iodev *iodev)
{
	const struct alsa_usb_io *aio = (const struct alsa_usb_io *)iodev;
...
int cras_alsa_usb_iodev_legacy_complete_init(struct cras_iodev *iodev)
{
	...
	if (direction == CRAS_STREAM_OUTPUT && is_first)
		cras_alsa_mixer_list_outputs(
			mixer, usb_new_output_by_mixer_control, aio);
	else if (direction == CRAS_STREAM_INPUT && is_first)
		cras_alsa_mixer_list_inputs(
			mixer, usb_new_input_by_mixer_control, aio);
	...
	usb_alsa_iodev_set_active_node(&aio->base,
				       first_plugged_node(&aio->base), 0);
		...
		if (output_node) {
			...
			if (!output_node->volume_curve)
				output_node->volume_curve =
					usb_create_volume_curve_for_jack(
						aio->config, jack);
		} else if (input_node) {
...
{
	struct alsa_usb_output_node *active =
		(struct alsa_usb_output_node *)ionode;
