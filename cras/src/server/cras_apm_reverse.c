...
static struct echo_ref_request *
find_echo_ref_request_for_dev(const struct cras_iodev *echo_ref)
{
...
static void reverse_data_run(struct ext_dsp_module *ext, unsigned int nframes)
{
	...
	while (nframes) {
		...
		if (!float_buffer_writable(rmod->fbuf)) {
			apm_process_reverse_callback(rmod->fbuf, rmod->dev_rate,
						     rmod->odev);
 ...
 */
static struct cras_apm_reverse_module *
create_apm_reverse_module(struct cras_iodev *odev)
{
	struct cras_apm_reverse_module *rmod;
...
static struct echo_ref_request *
create_echo_ref_request(struct cras_iodev *echo_ref)
{
...
static struct stream_apm_request *
create_stream_apm_request(struct cras_stream_apm *stream)
{
 ...
 */
static void
find_echo_ref_request_for_stream(struct cras_stream_apm *stream,
...
static void remove_stream_apm_request(struct echo_ref_request *request,
				      struct stream_apm_request *stream_apm_req)
{
