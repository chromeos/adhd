...
size_t cras_bt_adapter_get_list(struct cras_bt_adapter*** adapter_list_out) {
  ...
  DL_FOREACH (adapters, adapter) {
    ...
    tmp = realloc(adapter_list, sizeof(adapter_list[0]) * (num_adapters + 1));
    if (!tmp) {
      free(adapter_list);
      return -ENOMEM;
    }
...
static void bt_adapter_set_powered(struct cras_bt_adapter* adapter,
                                   int powered) {
...
static void on_get_supported_capabilities_reply(DBusPendingCall* pending_call,
  ...
  if (dbus_message_get_type(reply) == DBUS_MESSAGE_TYPE_ERROR) {
    syslog(LOG_WARNING, "GetSupportedCapabilities message replied error: %s",
  ...
  if (!dbus_message_iter_init(reply, &message_iter)) {
    syslog(LOG_ERR,
           "GetSupportedCapabilities reply doesn't have"
           "argument");
    goto get_supported_capabilities_err;
  }
  ...
  while (dbus_message_iter_get_arg_type(&capabilities) != DBUS_TYPE_INVALID) {
...
int cras_bt_adapter_get_supported_capabilities(
    struct cras_bt_adapter* adapter) {
  ...
  method_call = dbus_message_new_method_call(
      BLUEZ_SERVICE, adapter->object_path, BLUEZ_INTERFACE_ADAPTER,
  ...
  if (!dbus_connection_send_with_reply(adapter->conn, method_call,
    ...
    syslog(LOG_WARNING, "Failed to send GetSupportedCapabilities message");
    return -EIO;
  }

  dbus_message_unref(method_call);
  if (!dbus_pending_call_set_notify(
          pending_call, on_get_supported_capabilities_reply, adapter, NULL)) {
