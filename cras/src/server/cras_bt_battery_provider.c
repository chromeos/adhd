...
static char* address_to_device_path(const char* address) {
  size_t path_size = strlen(CRAS_DEFAULT_BATTERY_PREFIX) + strlen(address) + 1;
...
static struct cras_bt_battery* battery_new(const char* address,
                                           uint32_t level) {
...
static void populate_battery_properties(DBusMessageIter* iter,
  ...
  dbus_message_iter_open_container(&dict, DBUS_TYPE_DICT_ENTRY, NULL, &entry);
  dbus_message_iter_append_basic(&entry, DBUS_TYPE_STRING, &property_device);
  dbus_message_iter_open_container(&entry, DBUS_TYPE_VARIANT,
...
static struct cras_bt_battery* get_or_create_battery(
    struct cras_bt_battery_provider* provider,
    const char* address,
    uint32_t level) {
  struct cras_bt_battery* battery;
  DBusMessage* msg;
...
static void update_battery_level(
    const struct cras_bt_battery_provider* provider,
    struct cras_bt_battery* battery,
    uint32_t level) {
  DBusMessage* msg;
  ...
  msg = dbus_message_new_signal(battery->object_path, DBUS_INTERFACE_PROPERTIES,
                                DBUS_SIGNAL_PROPERTIES_CHANGED);

  dbus_message_iter_init_append(msg, &iter);
  dbus_message_iter_append_basic(&iter, DBUS_TYPE_STRING, &provider->interface);
  populate_battery_properties(&iter, battery);

  if (!dbus_connection_send(provider->conn, msg, NULL)) {
    syslog(LOG_WARNING,
           "Error sending " DBUS_SIGNAL_PROPERTIES_CHANGED " signal");
  }
...
static void on_bt_battery_changed(void* context,
                                  const char* address,
                                  uint32_t level) {
  struct cras_bt_battery_provider* provider = context;

  syslog(LOG_DEBUG, "Battery changed for address %s, level %d", address, level);

  if (!provider->is_registered) {
    syslog(LOG_WARNING,
           "Received battery level update while "
 ...
 * BlueZ.
 */
static void cras_bt_on_battery_provider_registered(
  ...
  if (dbus_message_get_type(reply) == DBUS_MESSAGE_TYPE_ERROR) {
    syslog(LOG_WARNING, "RegisterBatteryProvider returned error: %s",
...
int cras_bt_register_battery_provider(DBusConnection* conn,
  ...
  method_call = dbus_message_new_method_call(
      BLUEZ_SERVICE, adapter_path, BLUEZ_INTERFACE_BATTERY_PROVIDER_MANAGER,
  ...
  if (!dbus_pending_call_set_notify(pending_call,
                                    cras_bt_on_battery_provider_registered,
...
static void on_battery_provider_unregistered(DBusPendingCall* pending_call,
  ...
  if (dbus_message_get_type(reply) == DBUS_MESSAGE_TYPE_ERROR) {
    syslog(LOG_WARNING, "UnregisterBatteryProvider returned error: %s",
           dbus_message_get_error_name(reply));
  }
...
static int unregister_battery_provider(DBusConnection* conn,
  ...
  if (!dbus_pending_call_set_notify(
          pending_call, on_battery_provider_unregistered, NULL, NULL)) {
    dbus_pending_call_cancel(pending_call);
    dbus_pending_call_unref(pending_call);
    return -ENOMEM;
  }
...
static void cleanup_battery(struct cras_bt_battery_provider* provider,
  ...
  if (!dbus_connection_send(provider->conn, msg, NULL)) {
    syslog(LOG_WARNING,
           "Error sending " DBUS_SIGNAL_INTERFACES_REMOVED " signal");
  }
