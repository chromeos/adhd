...
static DBusHandlerResult cras_bt_endpoint_set_configuration(
  ...
  syslog(LOG_DEBUG, "SetConfiguration: %s", dbus_message_get_path(message));
...
static DBusHandlerResult cras_bt_endpoint_select_configuration(
  ...
  syslog(LOG_DEBUG, "SelectConfiguration: %s", dbus_message_get_path(message));
  ...
  if (len > sizeof(configuration) ||
    ...
    reply = dbus_message_new_error(
        message, "org.chromium.Cras.Error.UnsupportedConfiguration",
...
static DBusHandlerResult cras_bt_endpoint_clear_configuration(
  ...
  syslog(LOG_DEBUG, "ClearConfiguration: %s", dbus_message_get_path(message));
...
static DBusHandlerResult cras_bt_handle_endpoint_message(DBusConnection* conn,
  ...
  if (dbus_message_is_method_call(message, DBUS_INTERFACE_INTROSPECTABLE,
    ...
    return DBUS_HANDLER_RESULT_HANDLED;
                 ...
                 message, BLUEZ_INTERFACE_MEDIA_ENDPOINT, "SetConfiguration")) {
    return cras_bt_endpoint_set_configuration(conn, message, arg);

  } else if (dbus_message_is_method_call(message,
                                         BLUEZ_INTERFACE_MEDIA_ENDPOINT,
                                         "SelectConfiguration")) {
    return cras_bt_endpoint_select_configuration(conn, message, arg);
    ...
    return cras_bt_endpoint_clear_configuration(conn, message, arg);
                 ...
                 message, BLUEZ_INTERFACE_MEDIA_ENDPOINT, "Release")) {
...
int cras_bt_register_endpoint(DBusConnection* conn,
  ...
  method_call = dbus_message_new_method_call(
      BLUEZ_SERVICE, adapter_path, BLUEZ_INTERFACE_MEDIA, "RegisterEndpoint");
  if (!method_call) {
    return -ENOMEM;
  }
  ...
  key = "UUID";
  dbus_message_iter_open_container(&properties_array_iter, DBUS_TYPE_DICT_ENTRY,
                                   NULL, &properties_dict_iter);
  dbus_message_iter_append_basic(&properties_dict_iter, DBUS_TYPE_STRING, &key);
  dbus_message_iter_open_container(&properties_dict_iter, DBUS_TYPE_VARIANT,
  ...
  dbus_message_iter_append_basic(&properties_dict_iter, DBUS_TYPE_STRING, &key);
  dbus_message_iter_open_container(&properties_dict_iter, DBUS_TYPE_VARIANT,
                                   DBUS_TYPE_BYTE_AS_STRING, &variant_iter);
  ...
  dbus_message_iter_open_container(
      ...
      DBUS_TYPE_ARRAY_AS_STRING DBUS_TYPE_BYTE_AS_STRING, &variant_iter);
  ...
  dbus_message_iter_close_container(&message_iter, &properties_array_iter);
  ...
  if (!dbus_pending_call_set_notify(pending_call, cras_bt_on_register_endpoint,
                                    NULL, NULL)) {
...
int cras_bt_unregister_endpoint(DBusConnection* conn,
  ...
  method_call = dbus_message_new_method_call(
      BLUEZ_SERVICE, adapter_path, BLUEZ_INTERFACE_MEDIA, "UnregisterEndpoint");
  if (!method_call) {
    return -ENOMEM;
  }

  if (!dbus_message_append_args(method_call, DBUS_TYPE_OBJECT_PATH,
                                &endpoint->object_path, DBUS_TYPE_INVALID)) {
    dbus_message_unref(method_call);
    return -ENOMEM;
  }
...
int cras_bt_endpoint_add(DBusConnection* conn,
  ...
  if (!dbus_connection_register_object_path(conn, endpoint->object_path,
                                            &endpoint_vtable, &dbus_error)) {
    syslog(LOG_WARNING, "Couldn't register Bluetooth endpoint: %s: %s",
