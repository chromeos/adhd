...
static DBusHandlerResult cras_bt_profile_handle_new_connection(
  ...
  if (dbus_message_iter_get_arg_type(&message_iter) != DBUS_TYPE_UNIX_FD) {
  ...
  if (!device) {
    syslog(LOG_WARNING, "Device %s not found at %s new connection", object_path,
           profile_path);
...
static DBusHandlerResult cras_bt_handle_profile_messages(DBusConnection* conn,
  ...
  if (dbus_message_is_method_call(message, DBUS_INTERFACE_INTROSPECTABLE,
    ...
    return cras_bt_profile_handle_request_disconnection(conn, message, arg);
  } else if (dbus_message_is_method_call(message, BLUEZ_INTERFACE_PROFILE,
                                         "Cancel")) {
...
int cras_bt_register_profile(DBusConnection* conn,
  ...
  method_call = dbus_message_new_method_call(
      BLUEZ_SERVICE, PROFILE_MANAGER_OBJ_PATH, BLUEZ_PROFILE_MGMT_INTERFACE,
      "RegisterProfile");
  ...
  if (!append_key_value(&properties_array_iter, "Version", DBUS_TYPE_UINT16,
  ...
  if (profile->features &&
      !append_key_value(&properties_array_iter, "Features", DBUS_TYPE_UINT16,
                        DBUS_TYPE_UINT16_AS_STRING, &profile->features)) {
    dbus_message_unref(method_call);
    return -ENOMEM;
  }
  ...
  if (!dbus_pending_call_set_notify(pending_call, cras_bt_on_register_profile,
                                    NULL, NULL)) {
...
int cras_bt_add_profile(DBusConnection* conn, struct cras_bt_profile* profile) {
  ...
  if (!dbus_connection_register_object_path(conn, profile->object_path,
                                            &profile_vtable, &dbus_error)) {
...
int cras_bt_rm_profile(DBusConnection* conn, struct cras_bt_profile* profile) {
  ...
  if (!dbus_connection_unregister_object_path(conn, profile->object_path)) {
