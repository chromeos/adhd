...
static dbus_bool_t dbus_watch_add(DBusWatch* watch, void* data) {
  ...
  if ((flags & DBUS_WATCH_READABLE) && dbus_watch_get_enabled(watch)) {
    r = cras_system_add_select_fd(dbus_watch_get_unix_fd(watch),
                                  dbus_watch_callback, watch, POLLIN);
    if (r != 0) {
      return FALSE;
    }
  }
...
DBusConnection* cras_dbus_connect_system_bus() {
  ...
  if (!conn) {
    syslog(LOG_WARNING, "Failed to connect to D-Bus: %s", dbus_error.message);
  ...
  if (!dbus_connection_set_watch_functions(conn, dbus_watch_add,
                                           dbus_watch_remove,
                                           dbus_watch_toggled, NULL, NULL)) {
    goto error;
  }
  if (!dbus_connection_set_timeout_functions(
          conn, dbus_timeout_add, dbus_timeout_remove, dbus_timeout_toggled,
          NULL, NULL)) {
    goto error;
  }
