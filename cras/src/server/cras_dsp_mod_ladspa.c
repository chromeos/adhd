...
static int instantiate(struct dsp_module* module,
  ...
  if (!data->handle) {
    syslog(LOG_ERR, "instantiate failed for %s, rate %ld", desc->Label,
           sample_rate);
    return -ENOMEM;
  }
  return 0;
...
static void dump(struct dsp_module* module, struct dumper* d) {
  ...
  dumpf(d, "  LADSPA: dlopen=%p, desc=%p, handle=%p, activated=%d\n",
        data->dlopen_handle, data->descriptor, data->handle, data->activated);
  if (descriptor) {
    dumpf(d, "   Name=%s\n", descriptor->Name);
    dumpf(d, "   Maker=%s\n", descriptor->Maker);
  }
...
static int verify_plugin_descriptor(struct plugin* plugin,
  ...
  ARRAY_ELEMENT_FOREACH (&plugin->ports, i, port) {
    ...
    if ((port->direction == PORT_INPUT) != !!(port_desc & LADSPA_PORT_INPUT)) {
      syslog(LOG_ERR, "port direction mismatch: %s:%d!", plugin->title, i);
      return -EINVAL;
    }

    if ((port->type == PORT_CONTROL) != !!(port_desc & LADSPA_PORT_CONTROL)) {
      syslog(LOG_ERR, "port type mismatch: %s:%d!", plugin->title, i);
      return -EINVAL;
    }
  }
...
struct dsp_module* cras_dsp_module_load_ladspa(struct plugin* plugin) {
  ...
  if (!data->dlopen_handle) {
    syslog(LOG_ERR, "cannot open plugin from %s: %s", path, dlerror());
    goto bail;
  }
  ...
  if (!desc_func) {
    syslog(LOG_ERR, "cannot find descriptor function from %s: %s", path,
           dlerror());
    goto bail;
  }
  ...
  for (index = 0;; index++) {
    ...
    if (desc == NULL) {
      syslog(LOG_ERR, "cannot find label %s from %s", plugin->label, path);
      goto bail;
    }
    if (strcmp(desc->Label, plugin->label) == 0) {
      syslog(LOG_DEBUG, "plugin '%s' loaded from %s", plugin->label, path);
