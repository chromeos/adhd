...
static void value_set_string2(struct cras_expr_value* value,
                              const char* begin,
                              const char* end) {
  cras_expr_value_free(value);
...
static void function_not(cras_expr_value_array* operands,
  ...
  is_false = (value->type == CRAS_EXPR_VALUE_TYPE_BOOLEAN && !value->u.boolean);
...
static void function_and(cras_expr_value_array* operands,
  ...
  ARRAY_ELEMENT_FOREACH (operands, i, value) {
    ...
    if (value->type == CRAS_EXPR_VALUE_TYPE_BOOLEAN && !value->u.boolean) {
...
static void env_set_variable(struct cras_expr_env* env,
                             const char* name,
...
static void dump_value(struct dumper* d,
                       const struct cras_expr_value* value,
                       int indent) {
  switch (value->type) {
    case CRAS_EXPR_VALUE_TYPE_NONE:
...
void cras_expr_expression_eval(struct cras_expr_expression* expr,
  ...
  switch (expr->type) {
    ...
    case EXPR_TYPE_LITERAL:
      ...
      break;
    case EXPR_TYPE_VARIABLE: {
      struct cras_expr_value* value = find_value(env, expr->u.variable);
      if (value == NULL) {
        syslog(LOG_ERR, "cannot find value for %s", expr->u.variable);
      } else {
        copy_value(result, value);
      }
      break;
    ...
    case EXPR_TYPE_COMPOUND: {
      ...
      if (ARRAY_COUNT(&values) > 0) {
        ...
        if (f->type == CRAS_EXPR_VALUE_TYPE_FUNCTION) {
          ...
          syslog(LOG_ERR, "first element is not a function");
        }
      } else {
