...
static int floss_manager_register_callback(DBusConnection *conn)
{
	...
	if (!dbus_pending_call_set_notify(pending_call,
					  floss_manager_on_register_callback,
					  conn, NULL)) {
...
static int floss_get_managed_objects(DBusConnection *conn)
{
	...
	if (!dbus_pending_call_set_notify(
		    pending_call, floss_on_get_managed_objects, conn, NULL)) {
		dbus_pending_call_cancel(pending_call);
		dbus_pending_call_unref(pending_call);
		return -ENOMEM;
	}
...
static void floss_start(struct bt_stack *s)
{
	...
	if (!dbus_connection_add_filter(
		    s->conn, floss_handle_name_owner_changed, NULL, NULL) ||
	    !dbus_connection_add_filter(s->conn, floss_handle_interfaces_added,
					NULL, NULL) ||
	    !dbus_connection_add_filter(
		    s->conn, floss_handle_interfaces_removed, NULL, NULL))
...
static void floss_stop(struct bt_stack *s)
{
	...
	dbus_bus_remove_match(s->conn,
			      ...
			      "member='InterfacesRemoved'",
			      NULL);
	dbus_connection_remove_filter(s->conn, floss_handle_name_owner_changed,
				      NULL);
	dbus_connection_remove_filter(s->conn, floss_handle_interfaces_added,
				      NULL);
