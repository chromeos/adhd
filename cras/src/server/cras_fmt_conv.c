...
static void normalize(float** mtx, size_t m, size_t n, float factor) {
  ...
  if (factor <= 0) {
    syslog(LOG_ERR, "expect: normalize factor > 0, get: %f.", factor);
  }
...
static size_t quad_to_51(struct cras_fmt_conv* conv,
                         const uint8_t* in,
                         size_t in_frames,
                         uint8_t* out) {
  size_t fl, fr, rl, rr;
...
static size_t mono_to_71(struct cras_fmt_conv* conv,
                         const uint8_t* in,
                         size_t in_frames,
                         uint8_t* out) {
  size_t left, right, center;
...
static size_t stereo_to_71(struct cras_fmt_conv* conv,
                           const uint8_t* in,
                           size_t in_frames,
                           uint8_t* out) {
  size_t left, right, center;
...
static size_t quad_to_71(struct cras_fmt_conv* conv,
                         const uint8_t* in,
                         size_t in_frames,
                         uint8_t* out) {
  size_t fl, fr, rl, rr;
...
static size_t _51_to_71(struct cras_fmt_conv* conv,
                        const uint8_t* in,
                        size_t in_frames,
                        uint8_t* out) {
...
static size_t _51_to_quad(struct cras_fmt_conv* conv,
                          const uint8_t* in,
                          size_t in_frames,
                          uint8_t* out) {
...
static float** cras_internal_spk_channel_conv_matrix_create(
    const struct cras_audio_format* in,
  ...
  for (i = 0; i < CRAS_CH_MAX; i++) {
    for (j = 0; j < CRAS_CH_MAX; j++) {
      if (internal_spk_channel_mapping[i][j] && out->channel_layout[i] != -1 &&
          in->channel_layout[j] != -1) {
        mtx[out->channel_layout[i]][in->channel_layout[j]] +=
            internal_spk_channel_mapping[i][j];
      }
    }
  }
  normalize(
      mtx, out->num_channels, in->num_channels,
      normalize_factor(mtx[out->channel_layout[STEREO_L]], in->num_channels));
  return mtx;
}
...
struct cras_fmt_conv* cras_fmt_conv_create(const struct cras_audio_format* in,
  ...
  if (node_type == CRAS_NODE_TYPE_INTERNAL_SPEAKER && out->num_channels > 2 &&
      in->num_channels > 1) {
    conv->num_converters++;
    conv->ch_conv_mtx = cras_internal_spk_channel_conv_matrix_create(in, out);
    ...
    if (in->num_channels == 1 && out->num_channels == 2) {
    ...
    } else if (in->num_channels == 1 && out->num_channels == 8) {
      conv->channel_converter = mono_to_71;
    } else if (in->num_channels == 2 && out->num_channels == 8) {
      conv->channel_converter = stereo_to_71;
    } else if (in->num_channels == 4 && out->num_channels == 8) {
      conv->channel_converter = quad_to_71;
    } else if (in->num_channels == 6 && out->num_channels == 8) {
      conv->channel_converter = _51_to_71;
      ...
      if (in_channel_layout_set) {
        conv->ch_conv_mtx =
            cras_channel_conv_matrix_alloc(in->num_channels, out->num_channels);
        if (conv->ch_conv_mtx == NULL) {
          cras_fmt_conv_destroy(&conv);
          return NULL;
        }
        ...
        if (out->num_channels == 4) {
          surround51_to_quad_downmix_mtx(conv->ch_conv_mtx,
                                         conv->in_fmt.channel_layout);
        } else {
          surround51_to_stereo_downmix_mtx(conv->ch_conv_mtx,
                                           conv->in_fmt.channel_layout);
        }
      } else {
        if (out->num_channels == 4) {
      ...
      syslog(LOG_WARNING, "Using all_to_all map for %zu to %zu",
             in->num_channels, out->num_channels);
      conv->channel_converter = default_all_to_all;
    } else {
      syslog(LOG_WARNING, "Using some_to_some channel map for %zu to %zu",
             in->num_channels, out->num_channels);
  ...
  if (in->frame_rate != out->frame_rate) {
    ...
    syslog(LOG_DEBUG, "Convert from %zu to %zu Hz.", in->frame_rate,
           out->frame_rate);
    conv->speex_state =
        speex_resampler_init(out->num_channels, in->frame_rate, out->frame_rate,
                             SPEEX_QUALITY_LEVEL, &rc);
    if (conv->speex_state == NULL) {
      syslog(LOG_ERR, "Fail to create speex:%zu %zu %zu %d", out->num_channels,
             in->frame_rate, out->frame_rate, rc);
      cras_fmt_conv_destroy(&conv);
      return NULL;
    }
  }
  ...
  for (i = 0; i < conv->num_converters - 1; i++) {
    conv->tmp_bufs[i] =
        calloc(max_frames * 4 *  // width in bytes largest format.
...
void cras_channel_remix_convert(struct cras_fmt_conv* conv,
  ...
  for (fr = 0; fr < nframes; fr++) {
    for (ch = 0; ch < conv->in_fmt.num_channels; ch++) {
      tmp[ch] = s16_multiply_buf_with_coef(conv->ch_conv_mtx[ch], buf,
...
size_t cras_fmt_conv_in_frames_to_out(struct cras_fmt_conv* conv,
  ...
  if (conv->pre_linear_resample) {
    in_frames = linear_resampler_in_frames_to_out(conv->resampler, in_frames);
  }
...
size_t cras_fmt_conv_out_frames_to_in(struct cras_fmt_conv* conv,
  ...
  if (!conv->pre_linear_resample) {
    out_frames = linear_resampler_out_frames_to_in(conv->resampler, out_frames);
  }
...
size_t cras_fmt_conv_convert_frames(struct cras_fmt_conv* conv,
  ...
  if (pre_linear_resample) {
    ...
    if (conv->speex_state != NULL) {
      resample_limit =
          resample_limit * conv->in_fmt.frame_rate / conv->out_fmt.frame_rate;
      /*
  ...
  if (conv->speex_state != NULL) {
    ...
    if (post_linear_resample) {
      out_limit = linear_resampler_out_frames_to_in(conv->resampler, out_limit);
    }
  ...
  if (post_linear_resample) {
    ...
    fr_out = linear_resampler_resample(conv->resampler, buffers[buf_idx],
                                       &linear_resample_fr,
...
int cras_fmt_conversion_needed(const struct cras_fmt_conv* conv) {
  return linear_resampler_needed(conv->resampler) || (conv->num_converters > 1);
}
...
int config_format_converter(struct cras_fmt_conv** conv,
  ...
  syslog(LOG_DEBUG,
         ...
         "frames = %u",
         from->format, from->frame_rate, from->num_channels, target.format,
         target.frame_rate, target.num_channels, frames);
