...
static void cras_scale_buffer_inc_s16_le(uint8_t* buffer,
                                         unsigned int count,
                                         float scaler,
                                         float increment,
                                         float target,
...
static void cras_scale_buffer_s16_le(uint8_t* buffer,
                                     unsigned int count,
                                     float scaler) {
  int i;
  int16_t* out = (int16_t*)buffer;
...
static void cras_mix_add_s16_le(uint8_t* dst,
                                uint8_t* src,
                                unsigned int count,
                                unsigned int index,
                                int mute,
...
static void cras_mix_add_scale_stride_s16_le(uint8_t* dst,
                                             uint8_t* src,
...
static void cras_scale_buffer_inc_s24_le(uint8_t* buffer,
                                         unsigned int count,
                                         float scaler,
                                         float increment,
                                         float target,
...
static void cras_scale_buffer_s24_le(uint8_t* buffer,
                                     unsigned int count,
                                     float scaler) {
  int i;
  int32_t* out = (int32_t*)buffer;
...
static void cras_mix_add_s24_le(uint8_t* dst,
                                uint8_t* src,
                                unsigned int count,
                                unsigned int index,
                                int mute,
...
static void cras_mix_add_scale_stride_s24_le(uint8_t* dst,
                                             uint8_t* src,
                                             unsigned int dst_stride,
                                             unsigned int src_stride,
                                             unsigned int count,
  ...
  if (dst_stride == src_stride && dst_stride == 4) {
    for (i = 0; i < count; i++) {
      int32_t sum;
      if (need_to_scale(scaler)) {
        sum = *(int32_t*)dst + scale_s24_le(*(int32_t*)src, scaler);
      } else {
...
static void cras_scale_buffer_inc_s32_le(uint8_t* buffer,
                                         unsigned int count,
                                         float scaler,
                                         float increment,
                                         float target,
...
static void cras_scale_buffer_s32_le(uint8_t* buffer,
                                     unsigned int count,
                                     float scaler) {
  int i;
  int32_t* out = (int32_t*)buffer;
...
static void cras_mix_add_s32_le(uint8_t* dst,
                                uint8_t* src,
                                unsigned int count,
                                unsigned int index,
                                int mute,
...
static void cras_mix_add_scale_stride_s32_le(uint8_t* dst,
                                             uint8_t* src,
                                             unsigned int dst_stride,
                                             unsigned int src_stride,
                                             unsigned int count,
  ...
  if (dst_stride == src_stride && dst_stride == 4) {
    for (i = 0; i < count; i++) {
      int64_t sum;
      if (need_to_scale(scaler)) {
        sum = *(int32_t*)dst + *(int32_t*)src * scaler;
      } else {
        sum = *(int32_t*)dst + *(int32_t*)src;
      }
      if (sum > INT32_MAX) {
...
static void cras_scale_buffer_inc_s24_3le(uint8_t* buffer,
                                          unsigned int count,
                                          float scaler,
                                          float increment,
                                          float target,
...
static void cras_scale_buffer_s24_3le(uint8_t* buffer,
                                      unsigned int count,
                                      float scaler) {
  int32_t frame;
  int i;
...
static void cras_mix_add_s24_3le(uint8_t* dst,
                                 uint8_t* src,
                                 unsigned int count,
                                 unsigned int index,
                                 int mute,
...
static void cras_mix_add_scale_stride_s24_3le(uint8_t* dst,
                                              uint8_t* src,
                                              unsigned int dst_stride,
                                              unsigned int src_stride,
                                              unsigned int count,
...
static void scale_buffer_increment(snd_pcm_format_t fmt,
                                   uint8_t* buff,
                                   unsigned int count,
                                   float scaler,
                                   float increment,
  ...
  switch (fmt) {
    case SND_PCM_FORMAT_S16_LE:
      return cras_scale_buffer_inc_s16_le(buff, count, scaler, increment,
                                          target, step);
    case SND_PCM_FORMAT_S24_LE:
      return cras_scale_buffer_inc_s24_le(buff, count, scaler, increment,
                                          target, step);
    case SND_PCM_FORMAT_S32_LE:
      return cras_scale_buffer_inc_s32_le(buff, count, scaler, increment,
                                          target, step);
    case SND_PCM_FORMAT_S24_3LE:
      return cras_scale_buffer_inc_s24_3le(buff, count, scaler, increment,
                                           target, step);
    default:
      break;
  }
...
static void scale_buffer(snd_pcm_format_t fmt,
                         uint8_t* buff,
                         unsigned int count,
                         float scaler) {
  switch (fmt) {
...
static void mix_add(snd_pcm_format_t fmt,
                    uint8_t* dst,
                    uint8_t* src,
                    unsigned int count,
                    unsigned int index,
  ...
  switch (fmt) {
    case SND_PCM_FORMAT_S16_LE:
      return cras_mix_add_s16_le(dst, src, count, index, mute, mix_vol);
    case SND_PCM_FORMAT_S24_LE:
      return cras_mix_add_s24_le(dst, src, count, index, mute, mix_vol);
    case SND_PCM_FORMAT_S32_LE:
      return cras_mix_add_s32_le(dst, src, count, index, mute, mix_vol);
    case SND_PCM_FORMAT_S24_3LE:
      return cras_mix_add_s24_3le(dst, src, count, index, mute, mix_vol);
    default:
      break;
  }
}

static void mix_add_scale_stride(snd_pcm_format_t fmt,
                                 uint8_t* dst,
                                 uint8_t* src,
                                 unsigned int count,
                                 unsigned int dst_stride,
  ...
  switch (fmt) {
    case SND_PCM_FORMAT_S16_LE:
      return cras_mix_add_scale_stride_s16_le(dst, src, dst_stride, src_stride,
                                              count, scaler);
    case SND_PCM_FORMAT_S24_LE:
      return cras_mix_add_scale_stride_s24_le(dst, src, dst_stride, src_stride,
                                              count, scaler);
    case SND_PCM_FORMAT_S32_LE:
      return cras_mix_add_scale_stride_s32_le(dst, src, dst_stride, src_stride,
                                              count, scaler);
    case SND_PCM_FORMAT_S24_3LE:
      return cras_mix_add_scale_stride_s24_3le(dst, src, dst_stride, src_stride,
                                               count, scaler);
    default:
      break;
  }
