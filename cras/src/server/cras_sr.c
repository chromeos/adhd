			       ...
			       const size_t input_nbytes)
{
	...
	int rc = 0;
	sr->speex_state =
		speex_resampler_init(1, spec.input_sample_rate,
	...
	const unsigned int resampled_buf_size =
		cras_frames_at_rate(spec.input_sample_rate,
				    input_nbytes / sizeof(int16_t),
				    spec.output_sample_rate);
	if (sample_buffer_init(resampled_buf_size, sizeof(int16_t),
			       &sr->resampled)) {
					  ...
					  struct sample_buffer *input_buf)
{
	...
	while (num_queued_inputs > 0) {
		...
		speex_resampler_process_int(
			sr->speex_state, 0,
			(int16_t *)sample_buf_read_pointer(input_buf),
			&num_inputs_used,
			(int16_t *)sample_buf_write_pointer(&sr->resampled),
			&num_outputs);
...
static unsigned int
cras_sr_get_num_propagated(struct cras_sr *sr,
...
static void cras_sr_resampled_to_unprocessed(struct cras_sr *sr,
					     const unsigned int num_propagated)
{
	convert_s16le_to_f32le(
		(int16_t *)sample_buf_read_pointer(&sr->resampled),
		num_propagated,
...
static void cras_sr_unprocessed_to_processed(struct cras_sr *sr)
{
	...
	float *buf = (float *)sample_buf_read_pointer_size(&sr->internal,
							   &num_readable);
			      ...
			      struct sample_buffer *output_buf)
{
	...
	unsigned int num_need_propagated =
		MIN(sample_buf_queued(&sr->resampled),
		    sample_buf_available(output_buf));

	while (num_need_propagated > 0) {
		const unsigned int num_propagated = cras_sr_get_num_propagated(
			sr, output_buf, num_need_propagated);
		    ...
		    struct byte_buffer *output_buf)
{
	...
	unsigned int num_read_inputs =
		cras_sr_speex_process(sr, &input_sample_buf);
