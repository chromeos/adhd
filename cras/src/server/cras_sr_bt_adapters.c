...
cras_iodev_sr_bt_adapter_create(struct cras_iodev *iodev, struct cras_sr *sr)
{
	...
	if (!sr) {
		syslog(LOG_ERR,
		       "cras_iodev_sr_bt_adapter_create failed due to NULL sr.");
		return NULL;
	}
	...
	if (sample_buffer_init(DEFAULT_SAMPLE_BUFFER_SIZE /
				       cras_sr_get_frames_ratio(sr),
			       sizeof(int16_t), &adapter->input_buf) != 0) {
		syslog(LOG_ERR,
		       "cras_iodev_sr_bt_adapter_create failed due to input_buf init failure.");
	...
	if (sample_buffer_init(DEFAULT_SAMPLE_BUFFER_SIZE, sizeof(int16_t),
		...
		syslog(LOG_ERR,
		       "cras_iodev_sr_bt_adapter_create failed due to output_buf init failure.");
		goto cras_iodev_sr_bt_adapter_create_failed;
	}
 ...
 */
static unsigned
copy_mono_area_to_sample_buffer(const struct cras_audio_area *area,
				struct sample_buffer *buf)
{
	...
	for (int i = 0; i < 2 && remaining_frames; ++i) {
		...
		if (writable_frames == 0)
			break;
		const unsigned written_frames =
			MIN(remaining_frames, writable_frames);
...
static int
cras_iodev_sr_bt_adapter_propagate(struct cras_iodev_sr_bt_adapter *adapter,
				   unsigned frames)
{
	int rc = 0;
	...
	struct cras_iodev_sr_bt_adapter *adapter, struct timespec *tstamp)
{
	...
	if (can_invoke_model(adapter, tstamp)) {
		...
		cras_iodev_sr_bt_adapter_propagate(
			adapter,
			MIN(num_queued_sr_inputs,
			    cras_sr_get_num_frames_per_run(adapter->sr) /
					...
					unsigned *frames)
{
	...
	uint8_t *buf_ptr = sample_buf_read_pointer_size(&adapter->output_buf,
							&readable_frames);
...
int cras_iodev_sr_bt_adapter_put_buffer(
	struct cras_iodev_sr_bt_adapter *adapter, const unsigned nread)
{
