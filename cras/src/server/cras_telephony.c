...
static DBusHandlerResult handle_telephony_message(DBusConnection* conn,
  ...
  if (dbus_message_is_method_call(message, DBUS_INTERFACE_INTROSPECTABLE,
    ...
    dbus_message_unref(reply);
    return DBUS_HANDLER_RESULT_HANDLED;
  } else if (dbus_message_is_method_call(message, CRAS_TELEPHONY_INTERFACE,
                                         "IncomingCall")) {
...
void cras_telephony_start(DBusConnection* conn) {
  ...
  if (!dbus_connection_register_object_path(conn, CRAS_TELEPHONY_OBJECT_PATH,
                                            &control_vtable, &dbus_error)) {
    syslog(LOG_WARNING, "Couldn't register telephony control: %s: %s",
