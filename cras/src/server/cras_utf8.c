...
static const u8range_t kUTF8ByteSeqF0[] = {
    {0xf0, 0xf0}, {0x90, 0xbf}, {0x80, 0xbf}, {0x80, 0xbf}, {0, 0},
};

static const u8range_t kUTF8ByteSeqF1F3[] = {
    {0xf1, 0xf3}, {0x80, 0xbf}, {0x80, 0xbf}, {0x80, 0xbf}, {0, 0},
};

static const u8range_t kUTF8ByteSeqF4[] = {
    {0xf4, 0xf4}, {0x80, 0x8f}, {0x80, 0xbf}, {0x80, 0xbf}, {0, 0},
};
    ...
    if (!range || range->min == 0) {
      ...
      if (bom_chars < ARRAY_SIZE(kUTF8ByteOrderMask)) {
        if (byte == kUTF8ByteOrderMask[bom_chars]) {
           ...
           * no BOM. */
          bom_chars = ARRAY_SIZE(kUTF8ByteOrderMask);
          pos = string;
          continue;
        }
      }
      ...
      while (seq->ranges->min != 0) {
        if (byte >= seq->ranges->min && byte <= seq->ranges->max) {
          // Matching sequence.
          break;
        }
        seq++;
