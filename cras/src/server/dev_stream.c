...
unsigned int max_frames_for_conversion(unsigned int stream_frames,
  ...
  return MAX(
             ...
             cras_frames_at_rate(stream_rate, stream_frames, device_rate))
         /*
...
struct dev_stream* dev_stream_create(struct cras_rstream* stream,
  ...
  max_frames = max_frames_for_conversion(
      stream->buffer_frames, stream_fmt->frame_rate, dev_fmt->frame_rate);

  if (stream->direction == CRAS_STREAM_OUTPUT) {
    rc = config_format_converter(&out->conv, stream->direction, stream_fmt,
                                 dev_fmt, iodev->active_node->type, max_frames);
  } else {
    /*
  ...
  dev_frames =
      (stream->direction == CRAS_STREAM_OUTPUT)
          ? cras_fmt_conv_in_frames_to_out(out->conv, stream->buffer_frames)
          : cras_fmt_conv_out_frames_to_in(out->conv, stream->buffer_frames);

  out->conv_buffer_size_frames = 2 * MAX(dev_frames, stream->buffer_frames);
...
void dev_stream_set_dev_rate(struct dev_stream* dev_stream,
  ...
  if (dev_stream->dev_id == dev_stream->stream->main_dev.dev_id) {
    cras_fmt_conv_set_linear_resample_rates(dev_stream->conv, dev_rate,
                                            dev_rate);
  ...
  while (fr_written < num_to_write) {
    ...
    if (cras_fmt_conversion_needed(dev_stream->conv)) {
      ...
      dev_frames = cras_fmt_conv_convert_frames(
          dev_stream->conv, src, dev_stream->conv_buffer->bytes, &read_frames,
...
static unsigned int capture_with_fmt_conv(struct dev_stream* dev_stream,
  ...
  while (total_read < num_frames) {
    buffer = buf_write_pointer_size(dev_stream->conv_buffer, &write_frames);
...
static unsigned int capture_copy_converted_to_stream(
  ...
  stream_samples =
      cras_shm_get_writeable_frames(shm, cras_rstream_get_cb_threshold(rstream),
  ...
  while (total_written < num_frames) {
    converted_samples =
        buf_read_pointer_size(dev_stream->conv_buffer, &write_frames);
    ...
    cras_audio_area_config_buf_pointers(rstream->audio_area, &rstream->format,
                                        stream_samples);

    cras_audio_area_copy(rstream->audio_area, offset, &rstream->format,
                         dev_stream->conv_area, 0, software_gain_scaler);

    buf_increment_read(dev_stream->conv_buffer,
                       (size_t)write_frames * (size_t)frame_bytes);
    total_written += write_frames;
    cras_rstream_dev_offset_update(rstream, write_frames, dev_stream->dev_id);
    offset = cras_rstream_dev_offset(rstream, dev_stream->dev_id);
  }

  ATLOG(atlog, AUDIO_THREAD_CAPTURE_WRITE, rstream->stream_id, total_written,
...
unsigned int dev_stream_capture(struct dev_stream* dev_stream,
  ...
  if (cras_fmt_conversion_needed(dev_stream->conv)) {
    ...
    format_bytes =
        cras_get_format_bytes(cras_fmt_conv_in_format(dev_stream->conv));
    nread = capture_with_fmt_conv(
        dev_stream, area->channels[0].buf + area_offset * format_bytes,
        fr_to_capture);

    capture_copy_converted_to_stream(dev_stream, rstream, software_gain_scaler);
  } else {
    unsigned int offset = cras_rstream_dev_offset(rstream, dev_stream->dev_id);
    ...
    nread = cras_audio_area_copy(rstream->audio_area, offset, &rstream->format,
                                 area, area_offset, software_gain_scaler);

    ATLOG(atlog, AUDIO_THREAD_CAPTURE_WRITE, rstream->stream_id, nread,
          cras_shm_frames_written(shm));
    cras_rstream_dev_offset_update(rstream, nread, dev_stream->dev_id);
  }
...
int dev_stream_playback_frames(const struct dev_stream* dev_stream) {
  ...
  frames = cras_rstream_playable_frames(dev_stream->stream, dev_stream->dev_id);
  if (frames < 0) {
    return frames;
  }
...
unsigned int dev_stream_cb_threshold(const struct dev_stream* dev_stream) {
  ...
  if (rstream->direction == CRAS_STREAM_OUTPUT) {
    return cras_fmt_conv_in_frames_to_out(dev_stream->conv, cb_threshold);
  } else {
    return cras_fmt_conv_out_frames_to_in(dev_stream->conv, cb_threshold);
  }
}
...
unsigned int dev_stream_capture_avail(const struct dev_stream* dev_stream) {
  ...
  format_bytes =
      cras_get_format_bytes(cras_fmt_conv_out_format(dev_stream->conv));
  ...
  frames_avail =
      MIN(frames_avail, buf_available(dev_stream->conv_buffer) / format_bytes);
...
int dev_stream_capture_update_rstream(struct dev_stream* dev_stream) {
  ...
  ATLOG(atlog, AUDIO_THREAD_CAPTURE_POST, rstream->stream_id, frames_ready,
        rstream->shm->header->read_buf_idx);
...
int dev_stream_poll_stream_fd(const struct dev_stream* dev_stream) {
  ...
  if (!stream_uses_output(stream) || !cras_rstream_is_pending_reply(stream) ||
...
int dev_stream_wake_time(struct dev_stream* dev_stream,
  ...
  return get_input_wake_time(dev_stream, curr_level, level_tstamp, cap_limit,
                             is_cap_limit_stream, wake_time_out);
}
