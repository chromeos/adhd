...
static void output_mute_changed(void* context,
                                int muted,
                                int user_muted,
                                int mute_locked) {
...
size_t node_array_index_of_node_id(struct cras_ionode_info* nodes,
  ...
  for (i = 0; i < num_nodes; i++) {
    if (nodes[i].iodev_idx == dev_index && nodes[i].ionode_idx == node_index) {
      return i;
    }
  }
...
const char* node_name_for_node_id(struct cras_client* client,
  ...
  if (rc != 0) {
    ...
    snprintf(buf, sizeof(buf), "%u:%u", iodev_idx, node_index_of(node_id));
    return strdup(buf);
  }
  node_index = node_array_index_of_node_id(nodes, num_nodes, node_id);
  if (node_index >= num_nodes) {
    snprintf(buf, sizeof(buf), "unknown: %zu >= %zu", node_index, num_nodes);
  } else {
    snprintf(buf, sizeof(buf), "%u:%u: %s", nodes[node_index].iodev_idx,
...
int main(int argc, char** argv) {
  ...
  while (true) {
    ...
    option_character =
        getopt_long(argc, argv, "sl:", long_options, &option_index);
  ...
  cras_client_set_output_volume_changed_callback(client, output_volume_changed);
  cras_client_set_output_mute_changed_callback(client, output_mute_changed);
  cras_client_set_capture_mute_changed_callback(client, capture_mute_changed);
  cras_client_set_nodes_changed_callback(client, nodes_changed);
  cras_client_set_active_node_changed_callback(client, active_node_changed);
  ...
  cras_client_set_input_node_gain_changed_callback(client,
