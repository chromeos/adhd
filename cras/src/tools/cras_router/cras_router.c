...
static int run_file_io_stream(struct cras_client* client,
  ...
  *pfd = fd;
  ...
  aud_format =
      cras_audio_format_create(SND_PCM_FORMAT_S16_LE, rate, num_channels);
  if (aud_format == NULL) {
    close(pipefd[0]);
    close(pipefd[1]);
    free(pfd);
  ...
  params = cras_client_unified_params_create(
      direction, block_size, 0, 0, pfd, got_samples, stream_error, aud_format);
  if (params == NULL) {
    return -ENOMEM;
  }
  ...
  *pfd1 = loop_fd;
  ...
  loop_params = cras_client_unified_params_create(
      direction, block_size, 0, 0, pfd1, put_samples, stream_error, aud_format);
  stream_playing =
      start_stream(client, &loop_stream_id, loop_params, volume_scaler) == 0;
  if (!stream_playing) {
    return -EINVAL;
  }
...
int main(int argc, char** argv) {
  ...
  run_file_io_stream(client, pfd[1], pfd[0], CRAS_STREAM_INPUT, block_size,
                     rate, num_channels);
