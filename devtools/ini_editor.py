...
class Section(object):
    ...
    def InsertLine(self, line):
        ...
        if not to_insert:
            raise SectionException('The line %s does not match Port or NonPort syntax' % line)
    ...
    def InsertPort(self, port):
        ...
        if port.index > len(self.ports):
            raise SectionException(
                'Inserting port index %d but'
                ' currently there are only %d ports' % (port.index, len(self.ports))
            )
...
class IniEditor(object):
    ...
    def ParseArgs(self):
        ...
        """
        parser = argparse.ArgumentParser(description=('Edit or show the config files'))
        parser.add_argument(
            '--input_file',
            '-i',
            default=None,
        ...
        parser.add_argument(
            ...
            help='Config directory. By default it is ' '~/trunk/src/third_party/adhd/cras-config.',
        )
        parser.add_argument(
            '--board',
            '-b',
        ...
        parser.add_argument(
            '--section', '-s', default=None, help='The section to be shown/edited in the ini file.'
        )
        parser.add_argument(
            '--insert',
            '-n',
            default=None,
            help='The line to be inserted into the ini file. ' 'Must be used with --section.',
        )
        parser.add_argument(
            '--output-suffix',
            '-o',
        ...
        # directory.
            ...
            if self.args.config_dirs.startswith('~'):
                self.args.config_dirs = os.path.join(
                    os.path.expanduser('~'), self.args.config_dirs.split('~/')[1]
                )
            all_boards = os.walk(self.args.config_dirs).next()[1]
            # "board" argument must be a valid board name or "all".
            if not self.args.board or (
                self.args.board != ['all'] and not set(self.args.board).issubset(set(all_boards))
            ):
                logging.error(
                    'Please select a board from %s or use "all".' % (', '.join(all_boards))
                )
                raise IniEditorException('User must specify board if input_file ' 'is not given.')
    ...
    def PrintCommand(self, ini):
        ...
        if self.args.section:
            if ini.HasSection(self.args.section):
                ...
                logging.info('There is no section %s in %s', self.args.section, ini.file_path)
        else:
    ...
    def InsertCommand(self, ini):
        ...
        if self.args.output_suffix == 'None':
            answer = prompt('Writing output file in-place at %s ? [y/n]' % ini.file_path)
            if not answer:
                sys.exit('Abort!')
