...
func ProcessChange(gerritClient *gerrit.Client, change gerrit.ChangeInfo) {
	...
	if state == newState {
		...
		if _, _, err := gerritClient.Changes.SetReview(change.ID, change.CurrentRevision, review); err != nil {
		...
		if slices.Contains(change.Hashtags, audioQVTrigger) {
			if _, _, err := gerritClient.Changes.SetHashtags(change.ID, &gerrit.HashtagsInput{
		...
		_, _, err = gerritClient.Changes.SetReview(change.ID, change.CurrentRevision, review)
		if err != nil {
